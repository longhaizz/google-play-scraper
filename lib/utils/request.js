...
function doRequest (opts, limit) {
  ...
  // Don't attach the shared jar when the caller manages cookies itself (its own
  // cookieJar, or a login cookie header passed to version()) — got fills
  // headers.cookie from the jar on the second request onward, which would
  // otherwise silently clobber the caller's cookie. See got core index.js.
  const callerManagesCookies = opts.cookieJar ||
    (opts.headers && (opts.headers.cookie || opts.headers.Cookie));
  if (!callerManagesCookies) {
